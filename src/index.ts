...
/**
 * Check if the request origin is allowed based on environment configuration.
 * Reads ALLOWED_ORIGINS from env (comma-separated patterns) or falls back to DEFAULT_ALLOWED_ORIGINS.
 */
...
app.use('*', async (c, next) => {
  ...
  if (!origin) {
    ...
    allowedOrigin = '*';
  } else if (isAllowedOrigin(origin, c.env)) {
