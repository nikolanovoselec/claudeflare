...
export async function isMainAppAvailable(page: Page): Promise<boolean> {
  // Check if we have the header (main app) vs setup wizard
...
export async function waitForAppOrSetup(page: Page): Promise<'main' | 'setup'> {
  await page.waitForSelector('[data-testid="header-logo"], [data-testid="setup-wizard"], .setup-wizard, .wizard-container', {
    visible: true,
    timeout: DEFAULT_TIMEOUT,
  });
