...
import { launchBrowser, createPage, navigateToHome, takeScreenshot, waitForAppReady, waitForAppOrSetup, isMainAppAvailable, BASE_URL } from './setup';
import {
  waitForSelector,
  clickAndWait,
  getTextContent,
...
describe('Layout', () => {
  ...
  beforeAll(async () => {
    ...
    if (!mainAppAvailable) {
      console.log('⚠️ Main app not available (setup not complete). Layout tests will be skipped.');
    }
  });
  ...
  describe('Header', () => {
    ...
    it('logo contains Claudeflare text', async () => {
      if (skipIfNoMainApp()) return;
      await navigateToHome(page);
      await waitForAppReady(page);

      const logoText = await getTextContent(page, '[data-testid="header-logo"]');
      expect(logoText).toContain('Claudeflare');
    });
